...
@implementation CouponCommentView
...
- (void)setCoupon:(CouponInfo *)coupon {
    _coupon = coupon;
    if (coupon) {
...
- (void)_setupSubmitButton {
    self.submitButton = ({
        ...
        [btn addControlEvents:UIControlEventTouchUpInside action:^(UIControl *control, NSSet *touches) {
            ...
            if ([MKWStringHelper isNilOrEmptyString:tmp]) {
                [SVProgressHUD showErrorWithStatus:@"请输入评论内容"];
                return;
            }
            GCBlockInvoke(self.submitAction, self.commentTextView.text, self.coupon.couponId);
        }];
        btn;
    });
