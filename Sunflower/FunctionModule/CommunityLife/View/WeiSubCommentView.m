...
@implementation WeiSubCommentView
...
- (void)setComment:(WeiCommentInfo *)comment {
    _comment = comment;
    if (comment) {
...
- (void)_setupSubmitButton {
    self.submitButton = ({
        ...
        [btn addControlEvents:UIControlEventTouchUpInside action:^(UIControl *control, NSSet *touches) {
            ...
            NSNumber* parentCommentID = self.comment ? self.comment.commentId : @0;
