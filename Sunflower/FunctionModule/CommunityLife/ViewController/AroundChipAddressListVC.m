...
- (void)_setupObserver {
    ...
    [self startObserveObject:self forKeyPath:@"shop" usingBlock:^(NSObject *target, NSString *keyPath, NSDictionary *change) {
        _strong(self);
        [self.logoV setImageWithURL:[NSURL URLWithString:self.shop.image] placeholderImage:nil];
...
- (void)_loadCodingViews {
    ...
    self.shopTableV = ({
        ...
        [v withBlockForRowCell:^UITableViewCell *(UITableView *view, NSIndexPath *path) {
            ...
            [cell.logoV setImageWithURL:[NSURL URLWithString:self.logoUrlStr] placeholderImage:nil];
            return cell;
        }];
        v;
    });
